package com.google.android.gms.internal.p002firebaseauthapi;

import java.security.GeneralSecurityException;

/* compiled from: com.google.firebase:firebase-auth@@22.1.1 */
/* renamed from: com.google.android.gms.internal.firebase-auth-api.zzdf  reason: invalid package */
/* loaded from: classes4.dex */
final class zzdf extends zzlm {
    /* JADX INFO: Access modifiers changed from: package-private */
    public zzdf(Class cls) {
        super(cls);
    }

    @Override // com.google.android.gms.internal.p002firebaseauthapi.zzlm
    public final /* bridge */ /* synthetic */ Object zza(zzaii zzaiiVar) throws GeneralSecurityException {
        zzpg zzpgVar = (zzpg) zzaiiVar;
        return new zzvh((zzvz) new zzdv().zzl(zzpgVar.zze(), zzvz.class), (zzce) new zznp().zzl(zzpgVar.zzf(), zzce.class), zzpgVar.zzf().zzf().zza());
    }
}
