...
public class ByteOps {
    ...
    static {
        ...
        while (i4 < 2498) {
            int i5 = 16;
            int digit = (Character.digit(OPCODE_DETAILS.charAt(i4), 16) << 4) | Character.digit(OPCODE_DETAILS.charAt(i4 + 1), 16);
            char charAt = OPCODE_DETAILS.charAt(i4 + 3);
            if (charAt != '-') {
                if (charAt != 'I') {
                    if (charAt != 'W') {
                        if (charAt != 'p') {
            ...
            int i6 = i4 + 5;
            if (OPCODE_DETAILS.charAt(i6 - 1) == ':') {
                while (true) {
                    char charAt2 = OPCODE_DETAILS.charAt(i6);
                    if (charAt2 != 'D') {
                        if (charAt2 != 'F') {
                            if (charAt2 != 'c') {
                                if (charAt2 != 'f') {
            ...
            int indexOf = OPCODE_DETAILS.indexOf(59, i6);
            OPCODE_INFO[digit] = i5;
            OPCODE_NAMES[digit] = OPCODE_DETAILS.substring(i6, indexOf);
            i4 = indexOf + 1;
        }
    }
    ...
    public static int opInfo(int i4) {
        return OPCODE_INFO[i4];
    }

    public static String opName(int i4) {
        String[] strArr = OPCODE_NAMES;
        String str = strArr[i4];
        if (str == null) {
            String str2 = "unused_" + Hex.u1(i4);
            strArr[i4] = str2;
            return str2;
        }
        return str;
